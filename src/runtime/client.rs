...
impl RuntimeClient {
    ...
    pub fn snapshot(&self) -> Result<RuntimeRead<RuntimeSnapshot>> {
        self.read_with_alias(
            json!({"command": "runtime_snapshot"}),
            Some(json!({"command": "snapshot"})),
            false,
    ...
    pub fn dashboard(&self) -> Result<RuntimeRead<RuntimeDashboard>> {
        self.read_with_alias(
            json!({"command": "runtime_dashboard"}),
            Some(json!({"command": "desktop_dashboard"})),
            false,
    ...
    pub fn diagnostics(&self) -> Result<RuntimeRead<RuntimeDiagnostics>> {
        self.read_with_alias(
            json!({"command": "runtime_diagnostics"}),
            Some(json!({"command": "desktop_diagnostics"})),
            false,
    ...
    pub fn hygiene_preview(&self) -> Result<RuntimeRead<RuntimeHygienePreview>> {
        self.read_with_alias(
            json!({"command": "runtime_hygiene_preview"}),
            Some(json!({"command": "desktop_hygiene_preview"})),
            false,
    ...
    pub fn hygiene_archive(&self) -> Result<RuntimeRead<RuntimeHygieneArchive>> {
        self.read_with_alias(
            ...
            Some(json!({"command": "desktop_hygiene_archive"})),
            true,
    ...
    pub fn task_detail(&self, id: &str) -> Result<RuntimeRead<RuntimeTaskDetail>> {
        self.read_with_alias(
            ...
            Some(json!({"command": "desktop_task_detail", "id": id})),
            false,
    ...
    pub fn approval_detail(&self, id: &str) -> Result<RuntimeRead<RuntimeApprovalDetail>> {
        self.read_with_alias(
            ...
            Some(json!({"command": "desktop_approval_detail", "id": id})),
            false,
    ...
    pub fn checkpoint_detail(&self, id: &str) -> Result<RuntimeRead<RuntimeCheckpointDetail>> {
        self.read_with_alias(
            ...
            Some(json!({"command": "desktop_checkpoint_detail", "id": id})),
            false,
    ...
    fn read_with_alias<T, F>(
        &self,
        body: Value,
        legacy_body: Option<Value>,
        authed: bool,
        local: F,
    ) -> Result<RuntimeRead<T>>
    where
        T: DeserializeOwned,
        F: FnOnce(&RuntimeService) -> Result<T>,
    {
        self.read_inner_with_alias(body, legacy_body, authed, local)
    }
    ...
    where
        ...
        F: FnOnce(&RuntimeService) -> Result<T>,
    {
        self.read_inner_with_alias(body, None, authed, local)
    }

    fn read_inner_with_alias<T, F>(
        &self,
        body: Value,
        legacy_body: Option<Value>,
        authed: bool,
        ...
        F: FnOnce(&RuntimeService) -> Result<T>,
    {
        match self.mode {
            ...
            RuntimeClientMode::DaemonOnly => self
                .request_daemon_with_alias(body, legacy_body, authed)
                .map(|value| RuntimeRead {
                    source: RuntimeClientSource::Daemon,
                    value,
                }),
            RuntimeClientMode::PreferDaemon => {
                match self.request_daemon_with_alias(body, legacy_body, authed) {
    ...
    fn request_daemon_with_alias<T: DeserializeOwned>(
        &self,
        body: Value,
        legacy_body: Option<Value>,
        authed: bool,
    ) -> Result<T> {
        match self.request_daemon(body, authed) {
            Ok(value) => Ok(value),
            Err(primary) => {
                let Some(legacy_body) = legacy_body else {
                    return Err(primary);
                };
                self.request_daemon(legacy_body, authed)
            },
        }
    }
}
...
pub fn runtime_hygiene_reason() -> &'static str {
    "desktop runtime hygiene: test/dev artifact"
}
