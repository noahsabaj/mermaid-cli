...
use crate::constants::{
    PROXY_MAX_STARTUP_ATTEMPTS, PROXY_POLL_INTERVAL_MS, PROXY_STARTUP_WAIT_SECS,
};
...
pub async fn start_proxy() -> Result<()> {
    ...
    let max_wait_time = std::time::Duration::from_secs(
        PROXY_STARTUP_WAIT_SECS + (PROXY_MAX_STARTUP_ATTEMPTS as u64),
    );
    ...
    while start_time.elapsed() < max_wait_time {
        if is_proxy_running().await {
            ...
            log_info(
                "SUCCESS",
                format!(
                    "LiteLLM proxy started successfully in {:.1}s",
                    elapsed.as_secs_f64()
                ),
            );
            return Ok(());
...
pub async fn stop_proxy() -> Result<()> {
    let runtime = is_container_runtime_available()
        .ok_or_else(|| anyhow::anyhow!("No container runtime found (Podman or Docker)"))?;
    ...
    if !output.status.success() {
        ...
        log_warn(
            "WARNING",
            format!("Failed to stop LiteLLM proxy gracefully: {}", stderr),
        );
    } else {
