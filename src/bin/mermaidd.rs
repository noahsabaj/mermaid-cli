...
async fn handle_json_command(
    ...
    if command == "desktop_bootstrap" && !require_remote_auth {
        let store = mermaid_cli::runtime::RuntimeStore::open_default()?;
        let token_hash = str_field(body, "token_hash")?;
        let label = body.get("label").and_then(|v| v.as_str());
        if let Some(record) = store.pairing_tokens().get_enabled_by_hash(token_hash)? {
            return Ok(serde_json::json!({"ok": true, "pairing": record}));
        }
        let record = store.pairing_tokens().create(token_hash, label)?;
        return Ok(serde_json::json!({"ok": true, "pairing": record}));
    }
    ...
    match command {
        ...
        "desktop_dashboard" | "runtime_dashboard" => {
            let service = mermaid_cli::runtime::RuntimeService::open_default()?;
            Ok(serde_json::to_value(service.dashboard()?)?)
        },
        "desktop_diagnostics" | "runtime_diagnostics" => {
            let service = mermaid_cli::runtime::RuntimeService::open_default()?;
            Ok(serde_json::to_value(service.diagnostics()?)?)
        },
        "desktop_hygiene_preview" | "runtime_hygiene_preview" => {
            let service = mermaid_cli::runtime::RuntimeService::open_default()?;
            Ok(serde_json::to_value(service.hygiene_preview()?)?)
        },
        "desktop_hygiene_archive" | "runtime_hygiene_archive" => {
            let service = mermaid_cli::runtime::RuntimeService::open_default()?;
            Ok(serde_json::to_value(service.hygiene_archive()?)?)
        },
        "desktop_task_detail" | "runtime_task_detail" => {
            let service = mermaid_cli::runtime::RuntimeService::open_default()?;
            Ok(serde_json::to_value(
                service.task_detail(str_field(body, "id")?)?,
            )?)
        },
        "desktop_approval_detail" | "runtime_approval_detail" => {
            let service = mermaid_cli::runtime::RuntimeService::open_default()?;
            Ok(serde_json::to_value(
                service.approval_detail(str_field(body, "id")?)?,
            )?)
        },
        "desktop_checkpoint_detail" | "runtime_checkpoint_detail" => {
            let service = mermaid_cli::runtime::RuntimeService::open_default()?;
            Ok(serde_json::to_value(
