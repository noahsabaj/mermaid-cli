...
pub enum Cmd {
    ...
    /// Launch every subagent in `specs` in parallel. Each one runs its
    /// own nested reducer + effect runner; progress comes back as
    ...
    /// Persist default reasoning level.
    PersistDefaultReasoning(ReasoningLevel),
...
impl Cmd {
    ...
    pub fn is_turn_scoped(&self) -> bool {
        matches!(
            self,
            Cmd::CallModel { .. } | Cmd::ExecuteTool { .. } | Cmd::SpawnSubagents { .. }
        )
    }
    ...
    pub fn summary(&self) -> String {
        match self {
            ...
            Cmd::SpawnSubagents { turn, specs } => {
                format!("spawn_subagents(turn={}, n={})", turn, specs.len())
            },
            ...
            Cmd::PersistDefaultReasoning(l) => {
                format!("persist_default_reasoning({:?})", l)
            },
