//! The policy engine: `ActionRequest` in, `PolicyDecision` out.
//!
//! Folds the session's `SafetyMode`, the config overrides, the two blast-
...
mod tests {
    ...
    fn shallow_benign_nesting_is_not_over_blocked() {
        ...
        assert_eq!(classify_shell_command(cmd), RiskClass::ReadOnly);
    ...
    fn command_substitution_mutation_is_not_readonly() {
        ...
        assert_ne!(
            classify_shell_command("echo $(rm -rf ~/project/build)"),
            RiskClass::ReadOnly,
        ...
        assert_eq!(
            classify_shell_command("echo $(ls -la)"),
            RiskClass::ReadOnly,
    ...
    fn python_stdin_heredoc_classifies_by_the_consuming_command() {
        assert_eq!(
            classify_shell_command("python3 - <<'PY'\nprint(1)\nPY"),
            classify_shell_command("python3 -"),
        );
    }
    ...
    fn two_heredocs_consume_bodies_in_order() {
        assert_eq!(
            classify_shell_command("cat <<'A' <<'B'\nfirst body\nA\nsecond body\nB"),
            RiskClass::ReadOnly,
        );
    }
    ...
    fn arithmetic_shift_does_not_start_a_heredoc() {
        assert_eq!(
            classify_shell_command("echo $((1<<2))\ngit push"),
            RiskClass::Network,
        );
    }
    ...
    fn fd_prefixed_and_unterminated_heredocs_are_handled() {
        assert_eq!(
            classify_shell_command("cat 3<<'EOF'\nbody\nEOF"),
            RiskClass::ReadOnly,
        );
    ...
    fn wrapping_a_command_never_lowers_its_risk() {
        for base in [
            ...
            let bare = classify_shell_command(base);
            let wrapped = [
            ...
            for cmd in wrapped {
                let got = classify_shell_command(&cmd);
                assert!(
                    shell_severity(got) >= shell_severity(bare),
    ...
    fn safe_device_redirect_forms_stay_read_only() {
        for cmd in [
            ...
            assert_eq!(classify_shell_command(cmd), RiskClass::ReadOnly, "{cmd}");
    ...
    fn real_file_redirects_still_classify_as_writes() {
        ...
        assert_eq!(
            classify_shell_command("echo x > /dev/sda"),
            RiskClass::Destructive
        );
    }
    ...
    fn command_dash_v_lookup_is_read_only_but_command_exec_is_not() {
        ...
        assert_eq!(classify_shell_command("command -v rg"), RiskClass::ReadOnly);
        assert_eq!(classify_shell_command("command -v rm"), RiskClass::ReadOnly);
        assert_eq!(
            classify_shell_command("command -v rg >/dev/null 2>&1 && echo yes"),
            RiskClass::ReadOnly
        );
        assert_eq!(
            classify_shell_command("command rm -rf build"),
            RiskClass::ShellMutation
        );
        assert_eq!(classify_shell_command("command ls"), RiskClass::ReadOnly);
        assert_eq!(classify_shell_command("env -i ls"), RiskClass::ReadOnly);
        // Unknown token after wrapper flags still fails safe.
        assert_eq!(
            classify_shell_command("sudo -u web somethingunknown"),
            RiskClass::ShellMutation
        );
    }
    ...
    fn powershell_destructive_shapes_hard_denied() {
        ...
        for cmd in [
            ...
            assert!(!is_destructive_command(cmd), "must not hard-deny: {cmd}");
        }
    }
